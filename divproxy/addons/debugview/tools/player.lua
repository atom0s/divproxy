...
local tool = T{
    ...
    props = T{
        ...
        playersub = T{
            ...
            readonly        = T{ 'hwnd', },
            property_width  = 325,
            struct          = reflex.reflect('CPlayerSub'),
        },
    },
};
...
tool.render = function ()
    ...
    if (imgui.BeginTabBar(('%s_tab_bar'):fmt(tool.tag))) then
        if (imgui.BeginTabItem('Player')) then
            gui.render_property_editor(obj, tool.props.player);
            imgui.EndTabItem();
        end
        if (imgui.BeginTabItem('Player (Sub)')) then
            gui.render_property_editor(obj.unknown0000, tool.props.playersub);
            imgui.EndTabItem();
        end
        imgui.EndTabBar();
    end

...
local tool = T{
    ...
    props = T{
        manager = T{
            ...
            show_selection  = true,
            is_editable     = false,
            struct          = reflex.reflect('CSaveManager'),
        },
        quicksave = T{
            show_filters    = true,
            show_selection  = true,
            is_editable     = true,
            struct          = reflex.reflect('CSaveQuickInfo'),
        },
    },
};
...
tool.render = function ()
    ...
    local obj = game.get_save_manager();
    if (obj == nil) then
        return;
    end
    ...
    if (imgui.BeginTabBar(('%s_tab_bar'):fmt(tool.tag))) then
        ...
        if (imgui.BeginTabItem('Quick Save')) then
            gui.render_property_editor(obj.quick_info, tool.props.quicksave);
            imgui.EndTabItem();
        end
        imgui.EndTabBar();
    end
